# AGENTS.md - AI Agent Development Guide for tap-copernicus
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from Copernicus.

### Key Components

1. **Tap Class** (`tap_copernicus/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_copernicus/client.py`): Handles API communication and authentication
1. **Streams** (`tap_copernicus/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_copernicus/streams.py`
...
```python
class MyNewStream(CopernicusStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(CopernicusStream):
...
```bash
tap-copernicus --config config.json --discover
tap-copernicus --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_copernicus/tap.py`
...
```bash
# .env.example
TAP_COPERNICUS_API_URL=https://api.example.com
TAP_COPERNICUS_API_KEY=your_api_key_here
TAP_COPERNICUS_BATCH_SIZE=100  # New setting
```
